...
<div class="pt-32 bg-slate-200 text-black">
	...
	<div class="flex bg-white text-black">
		<div class="w-1/4"><CourseSideBar course={course}/></div>
		<div class="w-3/4 p-3 overflow-y-scroll max-h-screen">
			<div class="flex items-center"><Avatar src={course.avatar} classes="w-10 mr-3 rounded-full"/> {course.created_Name}</div>
			<hr class="my-5"/>
			<LessionVideoContainer lession={lession}/>
		</div>
	</div>
</div>
