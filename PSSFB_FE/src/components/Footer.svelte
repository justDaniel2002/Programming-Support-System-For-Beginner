<script>
    import BigLogBlack from "../assets/Đen 1.png"
</script>

<hr class="my-10"/>
<div class="bg-white text-black flex px-40 pb-10">
    <div class="w-2/5 mr-10">
        <img alt="blb" class="w-3/12" src="{BigLogBlack}"/>
        <div>PSSFB is an online platform that helps users to learn, practice coding skills and join the online coding contests.</div>
    </div>
    <div class="w-1/5">
        <div class="text-blue-500 text-xl font-medium mb-5">Links</div>
        <div class="mb-5">Learning</div>
        <div class="mb-5">Leaderboard</div>
    </div>
    <div class="w-1/5">
        <div class="text-blue-500 text-xl font-medium mb-5">information</div>
        <div class="mb-5">About Us</div>
    </div>
    <div class="w-1/5">
        <div class="text-blue-500 text-xl font-medium mb-5">Helps</div>
        <div class="mb-5">Help</div>
        <div class="mb-5">Forums</div>
        <div class="mb-5">Contact Us</div>
    </div>
</div>